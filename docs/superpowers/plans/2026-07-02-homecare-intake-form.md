...
- Email 通知收件者：`chiao1988ju@gmail.com`。
...
在 Drive 右鍵資料夾 → 共用 → 加 `chiao1988ju@gmail.com`（檢視者）。
...
`clasp deploy` 取臨時 URL，POST 一筆（收件者可先暫時改成自己的 email 測、確認後改回 `chiao1988ju@gmail.com`）。
